#!/usr/bin/env bash
# Download Dukascopy BTCUSD M1 BID candles as monthly CSVs.
set -euo pipefail

OUT_DIR="${1:-data/btcusd_m1}"
START="${2:-2022-01}"
END="${3:-2026-08}"
mkdir -p "$OUT_DIR"
...
while [ "$y" -lt "$ey" ] || { [ "$y" -eq "$ey" ] && [ "$m" -le "$em" ]; }; do
  ...
  dest=$(printf '%s/BTCUSD_M1_%04d_%02d.csv' "$OUT_DIR" "$y" "$m")
  if [ -f "$dest" ] && [ "$(wc -c < "$dest")" -gt 1000 ]; then
    echo "skip $(basename "$dest")"
  else
    echo "download $from -> $to"
    tmp=$(mktemp -d)
    npx --yes dukascopy-node -i btcusd -from "$from" -to "$to" -t m1 -f csv -dir "$tmp"
