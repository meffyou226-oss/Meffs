#!/usr/bin/env python3
"""Save already-trained m15 and m5 models, skip m1."""
...
def build_dataset(base_tf: str) -> pd.DataFrame:
    ...
    print(f"  shape={merged.shape} bal={merged['target'].mean():.2%} t={__import__('time').time()-t0:.1f}s", flush=True)
    return merged


def train_and_save(df: pd.DataFrame, base_tf: str):
    ...
    model_path = MODEL_DIR / f"lgb_price_action_{base_tf}_{ts}.txt"
    model.save_model(str(model_path))
    meta_path = MODEL_DIR / f"lgb_price_action_{base_tf}_{ts}_meta.json"
    with open(meta_path, "w") as f:
        json.dump(info | {"feature_cols": cols, "trained_at": ts}, f, indent=2, default=str)
    print(f"[{base_tf}] saved {model_path.name} acc={best_acc:.4f} auc={auc:.4f}", flush=True)
