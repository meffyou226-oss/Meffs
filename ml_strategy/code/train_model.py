#!/usr/bin/env python3
"""
Meffs XAUUSD Multi-Timeframe – Training Script (LOOKAHEAD-FIXED)
===============================================================
H1/M5 features are joined only after the bar is COMPLETE:
  available_at = bar_open + period_duration

See ml_strategy/results/LOOKAHEAD_BIAS_NOTICE.md
...
def build_multi_tf_frame(m15: pd.DataFrame, h1: pd.DataFrame, m5: pd.DataFrame) -> pd.DataFrame:
    """No lookahead: H1/M5 available only after bar close (open + period)."""
    ...
    m5_keep = [
        ...
        if c.startswith("m5_") and any(
            x in c for x in [
                "rsi_14", "rsi_7", "macd", "bb_pct", "atr_pct", "ema9_21",
...
def get_feature_columns(df: pd.DataFrame) -> list:
    ...
    cols = [
        ...
        or "pct" in c or "ema9_21" in c or "close_" in c
    ]
    return cols


def main():
    parser = argparse.ArgumentParser(description="Train Meffs XAUUSD multi-TF LightGBM (lookahead-fixed)")
    ...
    parser.add_argument("--train_end", type=str, default="2025-01-01")
    parser.add_argument("--val_end", type=str, default="2025-07-01")
    ...
    params = {
        ...
        "num_leaves": 47, "learning_rate": 0.025, "feature_fraction": 0.75,
        "bagging_fraction": 0.8, "bagging_freq": 5, "min_child_samples": 80,
        "reg_alpha": 0.2, "reg_lambda": 1.5, "verbose": -1, "n_jobs": -1, "seed": args.seed,
    }

    dtrain = lgb.Dataset(X_tr, y_tr)
    dval = lgb.Dataset(X_va, y_va, reference=dtrain)

    print("Training LightGBM (lookahead-fixed)...")
    model = lgb.train(
    ...
    acc_te, auc_te = evaluate("TEST (OOS)", X_te, y_te)

    model.save_model(str(out_dir / "lgb_xauusd_multi_tf.txt"))
    (out_dir / "feat_cols.json").write_text(json.dumps(feat_cols))
    print(f"Saved model + feat_cols to {out_dir}")
    ...
    if joblib is not None:
        joblib.dump({
            ...
            "meta": {
                ...
                "test_accuracy": float(acc_te), "test_auc": float(auc_te),
                "note": "H1/M5 only from completed bars",
            },
        }, out_dir / "lgb_xauusd_multi_tf.pkl", compress=3)
        print("Saved .pkl")

    print("\nDone. Expect ~50-53% test accuracy with this feature set (no leak).")
